...
\\
Merci à Dattaz, Fabbox, Micgeri971, Mossroy
Mozinet, Alterlibriste, Brieucs, Theo Chevalier et tous les autres que j'oublie...
\\
...
\justifying{
\item Les compilations maison de Mozilla pour le Flame ne sont plus compatibles avec l’Open C depuis un bon moment car la sous-couche n’est plus basée sur la même version d’Android
\item Les bugs ne sont pas du fait de l’équipe qui s’occupe de mettre à disposition les builds. Ils sont remontés aux développeurs et in ne peut faire autre chose qu’attendre une nouvelle compilation en espérant que le problème ait été réglé entre temps.
\item La fréquence des mises à jour, elle est donnée à titre indicatif mais ne doit pas être prise au pied de la lettre.
}
\end{itemize}
\end{block}

\begin{block}{Les limites des builds communautaires}
\begin{itemize}
\justifying{
\item Coupure Aléatoire FFOS V2.1 
\item Firefox 2.1, 2.2, problème d'heure non réglée au démarrage 
\item Accrochage-3G (se contournent sans mal en passant par le mode avion)
}
\end{itemize}
\end{block}
...
\begin{frame}
\frametitle{Liste non exhaustive des différences par rapport à la 2.1}
\begin{itemize}
\justifying{
\item Lors de la charge de la batterie, le \% ainsi que le temps restant est affiché sur l’écran de veille
\item Lors d’un appui long, les applis ouvertes sont en mode vignette les unes à côté des autres
\item Duck Duck Go est un des moteurs de recherche
\item On peut sélectionner, couper, copier, coller du texte d’une zone de saisie
\item On peut ouvrir une fenêtre de navigation privée
\item La vie privée peut être mieux réglée
\item Le wi-fi peut être ou non désactivé automatiquement lors de la mise en veille
 \item Les icônes de l’écran d’accueil peuvent être groupées
\item On peut agir sur l’appareil à distance pour le localiser, mettre un message, le bloquer ou effacer son contenu en cas de perte
\item Quelques effets d’animation
}
\end{itemize}
\end{frame}
...
\\
Tout le code source est disponbile sur Github.
}
\end{frame}
